...
namespace Linq
{
    ...
    public static class JoinOperations
    {
        ...
        public static IEnumerable<(string category, IEnumerable<Product> productsName)> GroupJoinQuery()
        {
            ...
            return categories.GroupJoin(products, category => category, prod => prod.Category,
        ...
        public static IEnumerable<(string category, string productName)> LeftOuterJoin()
        {
            ...
            return (
                from category in categories
                join prod in products on category equals prod.Category into joinedGroup
                from joined in joinedGroup.DefaultIfEmpty()
                select (category, joined?.ProductName ?? "(No products)")
            );
        }
    }
}