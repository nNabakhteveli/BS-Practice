...
namespace Linq
{
    ...
    public static class AggregationOperations
    {
        ...
        public static int CountOddNumbers()
        {
            ...
            return numbers.Count(num => num % 2 == 1);
        }
        ...
        public static IEnumerable<(string customerId, int orderCount)> CustomersOrdersCount()
        {
            ...
            var newList = new List<(string customerId, int orderCount)>();

            customers.ForEach(customer => newList.Add((customer.CustomerId, customer.Orders.Count())));
        ...
        public static IEnumerable<(string category, int productCount)> ProductsInCategoryCount()
        {
            ...
            var uniqueValues = new HashSet<(string category, int productCount)>();

            products.ForEach(product =>
                uniqueValues.Add(($"{product.Category}", products.Count(prod => prod.Category == product.Category))));
            return uniqueValues;
        }
        ...
        public static IEnumerable<(string category, int totalUnitsInStock)> TotalUnitsInStock()
        {
            ...
            return (
                ...
                into prods
                select (prods.Key, prods.Sum(p => p.UnitsInStock))
            );
        }
        ...
        public static IEnumerable<(string category, decimal cheapestPrice)> GetCheapestPrice()
        {
            ...
            var list = new List<(string category, decimal cheapestPrice)>();

            foreach (var item in products)
            {
                if (dict.ContainsKey(item.Category) == false || item.UnitPrice <= dict[item.Category])
                {
                    dict[item.Category] = item.UnitPrice;
                }
            }
        ...
        {
            List<Product> products = Products.ProductList;
            List<string> categories = products.Select(prod => prod.Category).Distinct().ToList();
            var filteredListOfProducts = new List<(string category, decimal cheapestPrice)>();

            foreach (var category in categories)
            {
                filteredListOfProducts.Add((category,
                    products.Where(product => product.Category == category).Average(price => price.UnitPrice)));
            }
        ...
        public static double SeededAggregate()
        {
            ...
            return attemptedWithdrawals.Aggregate((a, b) => (int)startBalance - a);
        }
    }
}